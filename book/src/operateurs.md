...
|---|---|---|
| `ops::mesher` | [Maillage](operateurs/maillage.md) | `line`, `circle`, `arc` (dispatché sous `circle`), `extrude`, `sweep`, `sweep_solid`, `translate`, `rotate`, `fill_surface`, `surface`, `volume`, `contour`, `elements_on`, `merge_nodes`, `read_gmsh`, `to_poi1`, `to_quadratic`, `barycenter`, `consolidate`… |
