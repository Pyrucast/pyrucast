...
| `ops::assemble` | [Assemblage](operateurs/assemblage.md) | `stiffness`, `mass`, rigidité géométrique `geometric`, concentration `lump`, chargement réparti `flux`, forces internes `internal_forces` / `internal_forces_continuum` (le `BSIG`, `∫ Bᵀ σ`) |
