...
| [`bernoulli`](mecanique/bernoulli.md)<br>`planar_1d` `frame_2d` `frame_3d` | — | — | — | `M`<br>`N, M`<br>`N, M_y, M_z, T` | Seule physique bâtie sur une interpolation **C¹** : elle exige un espace `HERMITE3`, dont la base cubique la rend **exacte aux nœuds** — un élément par barre suffit. Ne demande **ni `G` ni `A_s`** : réclamer une constante qu'une théorie n'utilise pas, c'est inviter la mauvaise. |
