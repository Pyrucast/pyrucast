...
mat1 = pc.build.material_field(mod1, [("k", 1)])
mesh_T_imp = pc.mesher.poi1_from_nodes([a])
lambda_mesh = pc.mesher.barycenter(mesh_T_imp)
pv = mod1[0].primal_vars()
dv = mod1[0].dual_vars()
mod1 = mod1 | pc.Model.dirichlet(pv[0], dv[0], mesh_T_imp, lambda_mesh)
K = pc.assemble.stiffness(mod1, mat1)
pa = pc.Mesh(c, "POI1")
pa[0].add_cell([b])
