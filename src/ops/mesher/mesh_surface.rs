...
/// may want to stop early, use [`mesh_surface_cancellable`].
pub fn mesh_surface(
    contour: &Mesh,
    element_type: ElementType,
    target_size: Option<f64>,
) -> Result<Mesh> {
    mesh_surface_cancellable(contour, element_type, target_size, &NoCancel)
}

/// Like [`mesh_surface`], but polls `cancel` periodically so meshing can be
/// stopped early (returning [`PyrucastError::Interrupted`]).
pub fn mesh_surface_cancellable(
    contour: &Mesh,
    element_type: ElementType,
    target_size: Option<f64>,
    cancel: &dyn Cancel,
    ...
    if !matches!(element_type, ElementType::TRI3 | ElementType::QUA4) {
        return Err(PyrucastError::Message(format!(
            "mesh_surface: element_type must be TRI3 or QUA4, got {}",
            element_type
        )));
    }
    if let Some(h) = target_size {
        if h <= 0.0 || h.is_nan() {
            return Err(PyrucastError::Message(format!(
                "mesh_surface: target_size must be > 0, got {}",
                h
            )));
        }
    }
    ...
    if dim != 2 && dim != 3 {
        return Err(PyrucastError::Message(format!(
            "mesh_surface: contour must be 2-D or 3-D, got dim={}",
            dim
        )));
    }
    ...
    if loops.is_empty() {
        return Err(PyrucastError::Message(
            "mesh_surface: contour has no boundary loop".into(),
        ));
    }
...
fn extract_loops(mesh: &Mesh) -> Result<Vec<LoopData>> {
    ...
    for sm in mesh {
        ...
        if s.element_type() != ElementType::SEG2 {
            return Err(PyrucastError::Message(format!(
                "mesh_surface: contour submeshes must be SEG2, got {}",
                s.element_type()
            )));
        }
        ...
        if n < 3 {
            return Err(PyrucastError::Message(
                "mesh_surface: a boundary loop needs at least 3 segments".into(),
            ));
        }
        ...
        for pair in conn.chunks(2) {
            if next.insert(pair[0], pair[1]).is_some() {
                return Err(PyrucastError::Message(
                    "mesh_surface: a boundary submesh is not a simple loop (branching)".into(),
                ));
            }
        }
        let start = conn[0];
        ...
        for _ in 0..n {
            if !seen.insert(cur) {
                return Err(PyrucastError::Message(
                    "mesh_surface: a boundary submesh is not a simple loop (repeated node)".into(),
                ));
            }
            chain.push(cur);
            cur = *next.get(&cur).ok_or_else(|| {
                PyrucastError::Message("mesh_surface: a boundary submesh is not closed".into())
            })?;
        }
        if cur != start {
            return Err(PyrucastError::Message(
                "mesh_surface: a boundary submesh is not closed".into(),
            ));
        }
...
impl Frame {
    fn fit(dim: u8, loops: &[LoopData]) -> Result<Frame> {
        ...
        if count == 0 {
            return Err(PyrucastError::Message("mesh_surface: empty contour".into()));
        }
        origin /= count as f64;
        ...
        if nn < 1e-30 {
            return Err(PyrucastError::Message(
                "mesh_surface: contour points are collinear or degenerate".into(),
            ));
        }
        normal /= nn;
...
fn build_domains(loops: Vec<Loop2D>) -> Result<Vec<Domain>> {
    ...
    for l in loops {
        ...
        if a.abs() < 1e-300 {
            return Err(PyrucastError::Message(
                "mesh_surface: a boundary loop has zero area".into(),
            ));
        }
        if a > 0.0 {
            outers.push(l);
    ...
    if outers.is_empty() {
        return Err(PyrucastError::Message(
            "mesh_surface: no counter-clockwise (outer) loop found".into(),
        ));
    }
    ...
    'hole: for h in holes {
        ...
        return Err(PyrucastError::Message(
            "mesh_surface: a hole (clockwise) loop is not contained in any outer loop".into(),
        ));
    }
    Ok(domains)
}
    ...
    fn corridor(&self, a: u32, b: u32) -> Result<(Vec<usize>, Vec<u32>, Vec<u32>)> {
        ...
        let (mut cur, mut p, mut q) = start_tri.ok_or_else(|| {
            PyrucastError::Message(
                "mesh_surface: could not recover a boundary edge (degenerate contour?)".into(),
            )
        })?;
        ...
        let mut le = self.local_edge_index(cur, p, q).ok_or_else(|| {
            PyrucastError::Message("mesh_surface: internal error recovering a boundary edge".into())
        })?;
        loop {
            let nb = self.tris[cur].nbr[le];
            if nb < 0 {
                return Err(PyrucastError::Message(
                    "mesh_surface: boundary edge recovery ran off the mesh (degenerate contour?)"
                        .into(),
                ));
            }
            let nb = nb as usize;
            ...
            let r =
                nt.v.iter()
                    ...
                    .ok_or_else(|| {
                        PyrucastError::Message(
                            "mesh_surface: internal error recovering a boundary edge".into(),
                        )
                    })?;
            crossed.push(nb);
            if r == b {
    ...
    fn refine(&mut self, h: f64, cancel: &dyn Cancel) -> Result<()> {
        ...
        while let Some(ti) = worklist.pop_front() {
            ...
            if created > max_new_points {
                return Err(PyrucastError::Message(
                    "mesh_surface: refinement did not converge within the point budget".into(),
                ));
            }
            let t = self.tris[ti];
            let (pa, pb, pc) = (
...
fn mesh_domain(
    ...
    if h <= 0.0 || h.is_nan() {
        return Err(PyrucastError::Message(
            "mesh_surface: could not determine a positive element size".into(),
        ));
    }
    ...
    if diag <= 0.0 {
        return Err(PyrucastError::Message(
            "mesh_surface: contour is degenerate (zero extent)".into(),
        ));
    }
    ...
    let (seed, _) = cdt
        ...
        .ok_or_else(|| {
            PyrucastError::Message(
                "mesh_surface: internal error locating the domain interior".into(),
            )
        })?;
...
fn materialize(
    ...
    if mesh.is_empty() {
        return Err(PyrucastError::Message(
            "mesh_surface: produced no cell".into(),
        ));
    }
    Ok(mesh)
}
...
mod tests {
    ...
    fn square_tri3_conserves_area() {
        ...
        let mesh = mesh_surface(&contour, ElementType::TRI3, Some(0.15)).unwrap();
    ...
    fn square_tri3_coarse_still_covers_boundary() {
        ...
        let coords = insert(Coords::new(2).unwrap());
        let contour = square(coords, 1.0);
        let mesh = mesh_surface(&contour, ElementType::TRI3, Some(10.0)).unwrap();
    ...
    fn square_qua4_conserves_area() {
        let coords = insert(Coords::new(2).unwrap());
        let contour = square(coords, 1.0);
        let mesh = mesh_surface(&contour, ElementType::QUA4, Some(0.2)).unwrap();
    ...
    fn two_disjoint_squares_are_independent_domains() {
        ...
        let mesh = mesh_surface(&contour, ElementType::TRI3, Some(0.3)).unwrap();
    ...
    fn rejects_bad_element_type() {
        let coords = insert(Coords::new(2).unwrap());
        let contour = square(coords, 1.0);
        assert!(mesh_surface(&contour, ElementType::TET4, None).is_err());
    }
    ...
    fn rejects_non_seg2_contour() {
        ...
        assert!(mesh_surface(&contour, ElementType::TRI3, None).is_err());
    }
    ...
    fn cancellable_stops_on_preset_flag() {
        ...
        let err =
            mesh_surface_cancellable(&contour, ElementType::TRI3, Some(0.1), &flag).unwrap_err();
    ...
    fn perf_hundred_k_triangles_under_30s() {
        ...
        let mesh = mesh_surface(&contour, ElementType::TRI3, Some(7.8e-4)).unwrap();
