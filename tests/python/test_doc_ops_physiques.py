...
def test_solve_eliminate():
    ...
    condense = pyrucast.solver.solve_eliminate(
        K, model, rhs
    )  # système réduit — même champ
...
def test_boucle_pas_a_pas():
    ...
    for step in range(1, nsteps + 1):
        ...
        out = pyrucast.element_field.integrate_behavior(
            model, eps, materials, prev=state
        )
