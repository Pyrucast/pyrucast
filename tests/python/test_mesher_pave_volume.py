"""Python tests for `pyrucast.mesh.pave_volume`."""
...
def test_pave_volume_makes_a_hex_layer_over_a_tet_core():
    mesh = pc.mesh.pave_volume(_box_skin(), layers=1, thickness=0.15, size=0.4)
    k = _cells(mesh)
    assert k["HEX8"] == 54, k
    assert k["PYRA5"] == 54, k
    assert k["TET4"] > 0, k
...
def test_pave_volume_stacks_several_layers():
    mesh = pc.mesh.pave_volume(_box_skin(), layers=2, thickness=0.08, size=0.4)
    k = _cells(mesh)
    assert k["HEX8"] == 108, k
    assert k["PYRA5"] == 54, k
