"""Python tests for `pyrucast.mesher.transfinite` (the DALL equivalent)."""
...
def _rectangle(nx, ny):
    ...
    side1 = pyrucast.mesher.line(p0, p1, nx)
    side2 = pyrucast.mesher.line(p1, p2, ny)
    side3 = pyrucast.mesher.line(p2, p3, nx)
    side4 = pyrucast.mesher.line(p3, p0, ny)
    return side1, side2, side3, side4


def test_transfinite_default_qua4():
    side1, side2, side3, side4 = _rectangle(4, 2)
    mesh = pyrucast.mesher.transfinite(side1, side2, side3, side4)
...
def test_transfinite_tri3():
    side1, side2, side3, side4 = _rectangle(4, 2)
    mesh = pyrucast.mesher.transfinite(side1, side2, side3, side4, element_type="TRI3")
...
def test_transfinite_rejects_mismatched_opposite_sides():
    ...
    p2 = c.add_node([2.0, 1.0])
    p3 = c.add_node([0.0, 1.0])
    side1 = pyrucast.mesher.line(p0, p1, 4)
    side2 = pyrucast.mesher.line(p1, p2, 2)
    side3 = pyrucast.mesher.line(p2, p3, 5)  # ≠ side1
    side4 = pyrucast.mesher.line(p3, p0, 2)
    try:
        pyrucast.mesher.transfinite(side1, side2, side3, side4)
    except RuntimeError:
        pass
    else:
...
def test_transfinite_unknown_element_type_raises():
    ...
    try:
        pyrucast.mesher.transfinite(side1, side2, side3, side4, element_type="BOGUS")
    except ValueError:
        pass
    else:
