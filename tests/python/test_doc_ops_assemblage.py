...
Chaque section de la page tire son bloc d'ici par `{{#include …:ancre}}`, et
pytest l'exécute : la page ne peut plus montrer un appel qui a cessé d'exister.
...
def test_assemble_apres_ajout_de_bloc():
    model, _, _, _ = _modele_thermique()
    materials = pyrucast.element_field.material_field(model, [("k", 1.0)])
