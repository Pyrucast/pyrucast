...
def test_mask_strict_vs_inclusive():
    ...
    assert _values(f.mask(gt=5.0)) == [0.0, 0.0, 1.0]  # 5 excluded
    assert _values(f.mask(ge=5.0)) == [0.0, 1.0, 1.0]  # 5 included
    assert _values(f.mask(lt=5.0)) == [1.0, 0.0, 0.0]  # 5 excluded
    assert _values(f.mask(le=5.0)) == [1.0, 1.0, 0.0]  # 5 included
...
def test_mask_component_filter_leaves_others_identity():
    ...
    m = f.mask(ge=0.0, le=5.0, components=["U"])
...
def test_mask_multiplies_to_zero_out_of_band():
    ...
    kept = f * f.mask(ge=0.0)  # zero the negatives
...
def test_comparison_equivalent_to_mask():
    f, _ = _poi1_field([[0.0], [5.0], [9.0]])
    assert _values(f > 5.0) == _values(f.mask(gt=5.0))
