...
def test_mask_keeps_structure_and_flags_band():
    ...
    m = pyrucast.field.mask(f, ge=10.0, le=30.0)
...
def test_mask_strict_vs_inclusive():
    ...
    assert _values(pyrucast.field.mask(f, gt=5.0)) == [0.0, 0.0, 1.0]  # 5 excluded
    assert _values(pyrucast.field.mask(f, ge=5.0)) == [0.0, 1.0, 1.0]  # 5 included
    assert _values(pyrucast.field.mask(f, lt=5.0)) == [1.0, 0.0, 0.0]  # 5 excluded
    assert _values(pyrucast.field.mask(f, le=5.0)) == [1.0, 1.0, 0.0]  # 5 included
...
def test_mask_is_per_component_no_and():
    ...
    m = pyrucast.field.mask(f, ge=0.0, le=5.0)
...
def test_mask_component_filter_leaves_others_identity():
    ...
    m = pyrucast.field.mask(f, ge=0.0, le=5.0, components=["U"])
...
def test_mask_multiplies_to_zero_out_of_band():
    ...
    kept = f * pyrucast.field.mask(f, ge=0.0)  # zero the negatives
...
def test_mask_requires_a_bound():
    ...
    with pytest.raises(RuntimeError):
        pyrucast.field.mask(f)


def test_mask_rejects_two_lower_bounds():
    f, _ = _poi1_field([[1.0]])
    with pytest.raises(RuntimeError):
        pyrucast.field.mask(f, ge=0.0, gt=0.0)
...
def test_comparison_equivalent_to_mask():
    f, _ = _poi1_field([[0.0], [5.0], [9.0]])
    assert _values(f > 5.0) == _values(pyrucast.field.mask(f, gt=5.0))
