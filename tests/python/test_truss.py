...
def test_truss_bar_axial_elongation():
    ...
    model = model | _clamp(model, c, n0, "u_x")
    model = model | _clamp(model, c, n0, "u_y")
    model = model | _clamp(model, c, n1, "u_y")  # bar has no transverse stiffness
